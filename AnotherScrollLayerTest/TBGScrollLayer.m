...
#import "TBGScrollLayer.h"


enum
{
	kTBGScrollLayerStateIdle,
	kTBGScrollLayerStateSliding,
};
...
-(id) initWithLayers:(NSArray *)layers pageSize:(CGSize)pageSize pagesOffset:(int)pOffset visibleRect:(CGRect)rect{
	if ( (self = [super init]) )
	{
		NSAssert([layers count], @"TBGScrollLayer#initWithLayers:widthOffset: you must provide at least one layer!");
...
-(void) moveToPage:(int)page
{
    if (page < 0 || page >= [layers_ count]) {
        CCLOGERROR(@"TBGScrollLayer#moveToPage: %d - wrong page number, out of bounds. ", page);
		return;
    }
...
-(BOOL) ccTouchBegan:(UITouch *)touch withEvent:(UIEvent *)event
{
	...
	state_ = kTBGScrollLayerStateIdle;
	return YES;
}
...
- (void)ccTouchMoved:(UITouch *)touch withEvent:(UIEvent *)event
{
	...
	if ( (state_ != kTBGScrollLayerStateSliding)
		&& (fabsf(touchPoint.y-startSwipe_) >= self.minimumTouchLengthToSlide) )
	{
		state_ = kTBGScrollLayerStateSliding;
